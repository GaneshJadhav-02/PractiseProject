...
Rails.application.configure do
  ...
  Rails.application.routes.default_url_options[:host] = ENV.fetch('APP_DOMAIN', nil)
  ...
  excluded_paths = ['/healthz', '/readyz'].freeze
  config.force_ssl = true
  config.ssl_options = {
    redirect: {
      exclude: ->(req) { excluded_paths.include?(req.path) }
    }
  }
  ...
  config.action_mailer.smtp_settings = {
    address: ENV.fetch('AWS_SES_SMPT_ADDRESS', nil),
    port: ENV.fetch('AWS_SES_SMPT_PORT', nil),
